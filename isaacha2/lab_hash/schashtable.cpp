...
void SCHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    table[hash(key,size)].push_back(pair<K,V>(key,value));
}
...
void SCHashTable<K, V>::resizeTable()
{
    ...
    while(i<size) {
        for (typename list<pair<K,V>>::iterator it = table[i].begin(); it != table[i].end(); it++) {
            newTable[hash(it->first,newSize)].push_back(*it);
        }
        i++;
    }
    delete[] table;
