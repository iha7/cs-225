...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto searchfirst = dict.find(first);
    auto searchsecond = dict.find(second);
    if(searchfirst != dict.end() && searchsecond != dict.end() && searchfirst->second == searchsecond->second) {
        return true;
    }
    return false;
}
