...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> search(KDTreeNode * subroot, const Point<Dim>& query, unsigned newDim) const;

    KDTreeNode * construct(std::vector<Point<Dim>>& points, unsigned newDim) const;

    bool inRadius(const Point<Dim>& potential, const Point<Dim>& target, const Point<Dim>& best, unsigned newDim) const;

    void destroy(KDTreeNode * subroot);

    unsigned partition(std::vector<Point<Dim>>& points, unsigned bottom, unsigned top, unsigned index, unsigned newDim) const;

    unsigned select(std::vector<Point<Dim>>& points, unsigned bottom, unsigned top, unsigned median, unsigned newDim) const;

    void swap(std::vector<Point<Dim>>& points, unsigned first, unsigned second) const;
