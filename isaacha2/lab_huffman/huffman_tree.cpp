                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* lowestNum = NULL;
    TreeNode* merge = NULL;
        if(!mergeQueue.empty()) {
        merge = mergeQueue.front();
    }
    if (!singleQueue.empty()) {
        lowestNum = singleQueue.front();
    }   
     if ( merge == NULL) {
        if(lowestNum != NULL) {
            singleQueue.pop();
    ...
    if ((lowestNum->freq).getFrequency() > (merge->freq).getFrequency()) {
        mergeQueue.pop();
        return merge;
    } else {
        singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    for (Frequency input : frequencies) {
        TreeNode* next = new TreeNode(input);
        singleQueue.push(next);
    }
    while(singleQueue.size() + mergeQueue.size() != 1) {
        ...
        TreeNode* subroot = new TreeNode((first->freq).getFrequency() + (second->freq).getFrequency());
        subroot->left = first;
        subroot->right = second;
        mergeQueue.push(subroot);
    }
if (singleQueue.empty() == false) {
    root_ = singleQueue.front();
} else if(mergeQueue.empty() == false) {
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        bool nextBit = bfile.getNextBit();
        if (nextBit == 1) {
            char leaf_char = bfile.getNextByte();
            TreeNode* out = new TreeNode(Frequency(leaf_char,0));
            return out;
        } else {
            TreeNode* out = new TreeNode(0);
            out->left = readTree(bfile);
            out->right = readTree(bfile);
            return out;
        }
    ...
    // HuffmanTree:: TreeNode* HuffmanTree:: readTree(TreeNode* current, BinaryFileReader& bfile) {
    //     if (bfile.hasBits) {
    //         bool nextBit = bfile.getNextBit();
    //         if (nextBit == 1) {
    //             TreeNode* out = new TreeNode(0);
